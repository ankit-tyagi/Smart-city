...
class User
{
	...
	function __construct($user,$display,$car,$mobile,$pass,$email,$location)
	{
		//Used for display only
		$this->displayname = $display;
		$this->user_location = sanitize($location);

		//Sanitize
		$this->clean_email = sanitize($email);
		$this->clean_password = trim($pass);
		$this->username = sanitize($user);
		$this->user_car = sanitize($car);
		$this->user_mobile = sanitize($mobile);
	...
	public function userCakeAddUser()
	{
		...
		if($this->status)
		{
			...
			if(!$this->mail_failure)
			{
				...
				$stmt->bind_param("ssssssssi", $this->username, $this->displayname, $this->user_car, $this->user_mobile, $secure_pass, $this->clean_email, $this->user_location, $this->activation_token, $this->user_active);
				$stmt->execute();
